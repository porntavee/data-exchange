...
@Component({
  selector: "app-trafficmonthly",
  templateUrl: "./trafficmonthly.component.html",
  styleUrls: ["./trafficmonthly.component.scss"]
})
export class TrafficmonthlyComponent implements OnInit {
  datacustomlist: any[] = [];
  ...
  constructor(
    ...
    this.titleService.setTitle("SED -Monthly Summary");
  }
  ...
  ngOnInit(): void {
    ...
    inputDatestart.setDate(inputDatestart.getDate());
  ...
  HistoryRowdata(data) {
    ...
    //   this.dialogHeader = "View Monthly Summary";
  ...
  exportPDF() {
    ...
    if (this.selecteddata.length != 0) {
      ...
      for (let i = 0; i < this.selecteddata.length; i++) {
        ...
        const fileName = `${this.selecteddata[i].circuit_id}_mtf_${this.filenameselect}_${createAt}.pdf`;
        ...
        const url = `https://edims.cathosting.in.th/report_service/report/customer_report?report_name=monthlysummary&ref_code=${this.selecteddata[i].ref_code}&start=${datefirstPickergetTime}&end=${datelastPickers}&file_type=pdf&user_create=test`;
        ...
        const body = JSON.stringify({
          ...
          ref_code: this.selecteddata[i].ref_code,
          start: datefirstPickergetTime,
          end: datelastPickers,
          filetype: "pdf"
        });
        fetch(url, {
  ...
  exportToExcel() {
    ...
    if (this.selecteddata.length != 0) {
      ...
      for (let i = 0; i < this.selecteddata.length; i++) {
        ...
        let userToken = localStorage.getItem("token");
        const fileName = `${this.selecteddata[i].circuit_id}_mtf_${this.filenameselect}_${createAt}.xlsx`;
        const downloadLink: HTMLAnchorElement = document.createElement("a");
        const url = `https://edims.cathosting.in.th/report_service/report/customer_report?report_name=monthlysummary&ref_code=${this.selecteddata[i].ref_code}&start=${datefirstPickergetTime}&end=${datelastPickers}&file_type=xlsx&user_create=test`;
        const authToken = userToken;
        const headers = new Headers({
          Authorization: `Bearer ${authToken}`
        });
        ...
        fetch(url, {
          ...
          .then(blob => {
            ...
            downloadLink.click();
            document.body.removeChild(downloadLink);
            const rowKey = this.selecteddata[i].id;
            this.exportState[rowKey] = false;
          })
          .catch(error => {
            const rowKey = this.selecteddata[i].id;
  ...
  exportToword() {
    ...
    if (this.selecteddata.length != 0) {
      this.selecteddata.forEach(selectedItem => {
        const rowKey = selectedItem.id;
        this.exportState[rowKey] = true;
      });
      for (let i = 0; i < this.selecteddata.length; i++) {
        const timestamp = this.getTimestamp();
        let userToken = localStorage.getItem("token");
        const fileName = `${this.selecteddata[i].circuit_id}_mtf_${this.filenameselect}_${createAt}.docx`;
        const downloadLink: HTMLAnchorElement = document.createElement("a");
        const url = `https://edims.cathosting.in.th/report_service/report/customer_report?report_name=monthlysummary&ref_code=${this.selecteddata[i].ref_code}&start=${datefirstPickergetTime}&end=${datelastPickers}&file_type=docx&user_create=test`;
        const authToken = userToken;
        const headers = new Headers({
          Authorization: `Bearer ${authToken}`
        });
