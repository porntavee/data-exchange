...
export class UserService {
    ...
    getUser() {
        ...
        return this.http.get<any>(environment.apiUrl+'/user/', {'headers':headers})
    ...
    editZone(id,symbol_id, name, description) {
        ...
        // console.log(body)
        return this.http.post<any>(`${environment.apiUrl}/user/edit_userzone?zone_id=${id}`, body, { 'headers': headers });
    }
    deleteZone(id) {

        const headers = { 'content-type': 'application/json' }
        const body = JSON.stringify
        return this.http.post<any>(`${environment.apiUrl}/user/delete_zone?zone_id=${id}`, body, { 'headers': headers });
    }
    getUserZone(){
        const headers = { 'accept': 'application/json'}
        return this.http.get<any>(environment.apiUrl+'/user/userzone', {'headers':headers})
    
    }
    getUserZoneAssign(){
        const headers = { 'accept': 'application/json'}
        return this.http.get<any>(environment.apiUrl+'/user/userzone_assign', {'headers':headers})
    ...
    creteuserzone(userzone){
        ...
        return this.http.post<any>(environment.apiUrl+'/user/userzone',body,{'headers':headers})
        
    }
    createUser(userdata){
        const headers = { 'content-type': 'application/json'}
        const body=JSON.stringify(userdata);
        return this.http.post<any>(environment.apiUrl+'/user/register',body,{'headers':headers})
    ...
    editUser(userdata: User){
        const headers = { 'content-type': 'application/json'}
        const body=JSON.stringify(userdata);
        return this.http.put<User>(environment.apiUrl+'/user/'+userdata.id,body,{'headers':headers});
    }

    deleteUsers(user_ids: number[]){
        const headers = { 'content-type': 'application/json'}
        const body=user_ids;
        return this.http.post<[]>(environment.apiUrl+'/user/delete',body,{'headers':headers});
    }
...
Device Type ISCOM2924GF-4C`
        ...
        return this.http.post<any>(`${environment.apiUrl}/send_email`,body,{'headers':headers});
    }
}